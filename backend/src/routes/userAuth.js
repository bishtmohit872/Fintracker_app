...
userAuth.post('/login', async (req, res) => {
    try {
        ...
        if (isPasswordValid) {
            ...
            res.cookie("token", token, { expires: new Date(Date.now() + 8 * 3600000), httpOnly:false,secure:true,sameSite:'None'})
            res.send(
                {
...
userAuth.post("/logout", async (req, res) => {
    ...
    res.cookie("token", "", {
        expires: new Date(0),
        path: "/",       // same as login
        httpOnly: false, // match with login
        secure: false,   // match with login
        sameSite: 'Lax'  // optional, if Brave is strict
    });
